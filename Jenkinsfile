...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="stingls/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/sejong95lsj/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/sejong95lsj/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://sejong95lsj:${GITHUB_ACCESS_TOKEN}@github.com/sejong95lsj/guestbook-config.git"
                    '''
                }
            }
        }
